...
def create_toss_pending():
    ...
    print(f'[Toss] 결제 요청 생성: {payment_id}, store={store_id}, table={data.get("table_id")}, ws={ws_address}', flush=True)
    print(f'[Toss] order data: {_json.dumps(data.get("order"), ensure_ascii=False)}', flush=True)
    _sys.stdout.flush()
