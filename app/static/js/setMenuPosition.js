...
const createHtml = (menuPageData) => {
  ...
  // 인덱스 범위가 넘어갈 경우를 대비한 안전 코드
  if (!menuPageData[indexData.main]) {
    indexData.main = 0;
  }
  ...
  if (!subCategoryData[indexData.sub]) {
    indexData.sub = 0;
  }
  ...
  const currentPageData = subCategoryData[indexData.sub]?.pageList ?? [];
  currentPageData.sort((a, b) => a.page - b.page);

  if (!currentPageData[indexData.page]) {
    indexData.page = 0;
  }

  const menuListData = currentPageData[indexData.page]?.menuList ?? [];
...
const clickDeleteCategoryItem = async (event, type) => {
  ...
  const _ul = _li.parentElement;

  if (_ul.querySelectorAll('li').length <= 1) {
    return showToast(`최소 한 개의 ${type == 'MAIN' ? '메인' : '서브'} 카테고리는 남아있어야 합니다.`, 'error');
  }
  ...
  if (id) { // 이용 중인 메뉴 카테고리가 있는지 확인하는 api 통신
    ...
    if (result.status) {
      ...
      showToast('식사 중인 메뉴가 포함된 카테고리는 삭제할 수 없습니다.', 'error')
    }
  } else {
    _li.remove();
  }
...
const clickSetMenuCategoryData = async (event, type) => {
  ...
  if (__li.length === 0) {
    return showToast(`최소 한 개의 ${type == 'MAIN' ? '메인' : '서브'} 카테고리가 필요합니다.`, 'error');
  }
